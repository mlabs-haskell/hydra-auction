...
This section describes an extenion that transforms the auctions from being closed to open.

This means that, rather than having a fixed number of participants that must be decided beforehand,
...
a pair consisting of the `hydraHeadId` and `bidderPKH`

```haskell
import Crypto.Sign.Ed25519

generateSellerSignature :: SigningKey PaymentKey -> HeadId -> PubKeyHash -> BuiltinByteString
generateSellerSignature skey headId bidderPKH = toBuiltin $ dsign skey $ sellerSignatureMessage headId bidderPKH 

sellerSignatureMessage :: HeadId -> PubKeyHash -> BuiltinByteString
sellerSignatureMessage headId bidderPKH = toByteString headId <> toBysteString bidderPKH
```
...
validNewBid AuctionTerms{..} oldStandingBidState newStandingBidState =
  isJust $ do
    ...
    guard $ verifyEd25519Signature (sellerVK terms) (sellerSignatureMessage (hydraHeadId terms) bidderPKH) sellerSignature
    -- The new bid respects the starting price and minimum bid increment
    -- conditions in the auction terms.
    guard $ case oldBid of
      Nothing ->
...
for enstablishing the on-chain link between bidder verification key and pubkey hash
...
```haskell
generateBidderSignature :: SigningKey PaymentKey -> HeadId -> Natural -> PubKeyHash -> BuiltinByteString
generateBidderSignature skey headId bidAmount bidderPKH = toBuiltin $ dsign skey $ bidderSignatureMessage headId bidderPKH bidAmount

bidderSignatureMessage :: HeadId -> Natural -> PubKeyHash -> BuiltinByteString
bidderSignatureMessage headId bidAmount bidderPKH = toByteString headId <> toBysteString bidderPKH
```
...
```haskell
generateSellerSignature :: SigningKey PaymentKey -> HeadId -> PubKeyHash -> SigningKey VerificationKey ->  BuiltinByteString
generateSellerSignature skey = toBuiltin . (dsign skey) . sellerSignatureMessage

sellerSignatureMessage :: HeadId -> PubKeyHash -> SigningKey VerificationKey -> BuiltinByteString
sellerSignatureMessage headId bidderPKH bidderVK = toByteString headId <> toBysteString bidderPKH <> toByteString bidderVK
```
...
validNewBid AuctionTerms{..} oldStandingBidState newStandingBidState =
  isJust $ do
    ...
    -- The seller has allowed the bidder to participate in the auction
    guard $ verifyEd25519Signature (sellerVK terms) (sellerSignatureMessage (hydraHeadId terms) bidderPKH bidderVK) sellerSignature
    -- The bidder has correctly signed the datum
    guard $ verifyEd25519Signature bidderVK (bidderSignatureMessage (hydraHeadId terms) newPrice bidderPKH) bidderSignature
    -- The new bid respects the starting price and minimum bid increment
    -- conditions in the auction terms.
    guard $ case oldBid of
      Nothing ->
