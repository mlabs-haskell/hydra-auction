...
main = do
  ...
  let config = fromMaybe (error "Cannot decode config.json") (decode configBytes)
  ...
  case action of
    ...
    ShowScriptUtxos script actor utxoRef -> do
      terms <- termsFromConfig config actor utxoRef
    ...
    AuctionAnounce actor utxo -> do
      terms <- termsFromConfig config actor utxo
